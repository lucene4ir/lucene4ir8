...
def findExSet (aKey):
    ...
    global GCsvInPath , GYAxis , GFBDocs , GCorpus
    ...
    for line in f :
        ...
        if corpus == GCorpus and fbdocs == GFBDocs and coefficient == 5.1:
            found = True
            result.append(line)
        elif found:
            break
...
def inputKey(index):
    ...
    # corpus, indexType, qryFilter, qryCount, model, MaxResults , b ,
    global GCorpus , GIndexType , GModel
    ...
    key = sep.join([corpus, indexType, qryFilter, qryCount, model, c]) + sep
    return key
...
def getGroupIndex (index):
    ...
    if (index == modelCoefficientIndex ):
        ...
        result = modelCoefficientFileIndex - 2
    elif (group == 'Per'):
        result = index + 3
    elif (index == rSumIndex):
        result = GIndex + 2
...
def getFigName():
    ...
    if (corpus == "" or model == "" or xName == "" or yName == ""):
        ...
        result = '/' + "-".join([corpus,model,'c5.1' , xName,yName]) + ".png"
    else:
