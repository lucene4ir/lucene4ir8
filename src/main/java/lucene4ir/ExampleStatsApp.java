...
public class ExampleStatsApp {
    ...
    public void iterateThroughDocListAll()  throws IOException {
        ...
        for (int i = 0; i < n; i++) {
            ...
            // the doc.get pulls out the values stored - ONLY if you store the fields
            String docnum = doc.get("docnum");
            String all = doc.get(Lucene4IRConstants.FIELD_RAW).trim();
            if (all.length() == 0) {
    ...
    public void countFieldData() throws IOException {
        ...
        for (int i = 0; i < n; i++) {
            ...
            String content = doc.get(Lucene4IRConstants.FIELD_BODY);
            if (title.length()>0){
                nt++;
            }
            if (content.length()>0){
    ...
    public void extractBigramsFromStoredText() throws IOException {
        ...
        for (int i = 0; i < n; i++) {
            ...
            Document doc = reader.document(i);
            String all = doc.get(Lucene4IRConstants.FIELD_RAW);
